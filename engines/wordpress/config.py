...
config.py
=========
Loads and validates the YAML configuration file for the WordPress Directory Scraper.
...
Responsibilities:
  ...
  - Injecting runtime secrets from environment variables so they never
    need to be committed to version control

Supported environment variable overrides:
  SCRAPER_COOKIES_RAW → config["cookies_raw"]  (session cookie string)
  SCRAPER_PROXY       → config["proxy"]        (HTTP/HTTPS proxy URL)
"""
...
def load_config(config_path: str = "config.yaml") -> Dict[str, Any]:
    ...
    top-level key is present, then applies environment variable overrides
    before returning.
...
def _apply_env_overrides(config: Dict[str, Any]) -> None:
    ...
    that secrets (cookies, proxy credentials) are never committed to Git.

    Args:
        config: Mutable config dict (modified in-place).
    """
    cookies = os.environ.get("SCRAPER_COOKIES_RAW", "")
    if cookies:
        config["cookies_raw"] = cookies
        log.debug("SCRAPER_COOKIES_RAW injected from environment.")

    proxy = os.environ.get("SCRAPER_PROXY", "")
    if proxy:
        config["proxy"] = proxy
        log.debug("SCRAPER_PROXY injected from environment.")
